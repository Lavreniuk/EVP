...
def main(args):
    ...
    for name, m in single_model.named_parameters():
        ...
        elif 'unet' in name or 'clip' in name:
    ...
    min_lr = 1e-7
    lr_scheduler = CosineLRScheduler(optimizer, t_initial=args.epochs, lr_min=min_lr,
                                            warmup_t=1, warmup_lr_init=min_lr, warmup_prefix=True)
    # housekeeping
    ...
    if utils.is_main_process():
        wandb.init(project='EVP', name=args.model_id, config=args)
        wandb.define_metric("custom_step")
        wandb.define_metric("validation_ref/loss", step_metric='custom_step')
    ...
    for epoch in range(max(0, resume_epoch+1), args.epochs):
        ...
        if utils.is_main_process():
            wandb.log({"custom_step": epoch, "validation_ref/loss": overallIoU})
        if save_checkpoint:
            print('Better epoch: {}\n'.format(epoch))
