...
def load_model(ckpt, model, optimizer=None):
    ...
    if optimizer is not None and 'optimizer' in ckpt_dict:
...
def main():
    ...
    if args.rank == 0:
        ...
        wandb.init(project='EVP', name=exp_name, config=args)
    ...
    for epoch in range(start_ep, args.epochs + 1):
        ...
        if epoch % args.val_freq == 0:
            ...
            if args.rank == 0:
                ...
                wandb.log({"custom_step": epoch, "validation/loss": results_dict[metric]})
        ...
        if args.rank == 0:
            ...
            if results_dict[metric] < best_rmse:
                best_rmse = results_dict[metric]
                torch.save(
                    {
                        'model': model_without_ddp.state_dict(),
                    },
...
def train(train_loader, model, criterion_d, log_txt, optimizer, device, epoch, args):    
    ...
    scheduler = CosineLRScheduler(optimizer, t_initial=args.epochs, lr_min=args.min_lr, 
                                warmup_t=1, warmup_lr_init=args.min_lr, warmup_prefix=True)
    for batch_idx, batch in enumerate(train_loader):      
        ...
        current_lr = scheduler._get_lr(global_step/iterations)[0]
        #print('*'*100)
        ...
        preds = model(input_RGB, class_ids=batch['class_id'], img_paths=batch['img_path'])

        optimizer.zero_grad()
        loss_d = criterion_d(preds['pred_d'].squeeze(dim=1), depth_gt)
        #loss_rl_d = criterion_d(preds['pred_rl_d'].squeeze(dim=1), depth_gt)
        #loss_d = 0.5*loss_d + 0.5*loss_rl_d
        ...
        if args.rank == 0:
            ...
            if batch_idx % args.print_freq == 0:
                ...
                print(result_line)
                #print(model.module.encoder.a, model.module.encoder.b)
                wandb.log({"train/loss": depth_loss.avg}, step=(epoch-1)*iterations + batch_idx)
        optimizer.step()
    ...
    for batch_idx, batch in enumerate(val_loader):
        ...
        with torch.no_grad():
            ...
            if args.flip_test:
                ...
                #img_paths = torch.cat((img_paths, img_paths), dim=0)
                img_paths.extend(img_paths)
            pred = model(input_RGB, class_ids=class_ids, img_paths=img_paths)
        pred_d = pred['pred_d']
        if args.flip_test:
