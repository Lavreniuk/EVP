...
#        assert class_id >= 0
        ...
        depth_mask = np.load(gt_path.replace('.npy','_mask.npy'))
        
        if self.scale_size:
            image = cv2.resize(image, (self.scale_size[0], self.scale_size[1]))
            depth = cv2.resize(depth, (self.scale_size[0], self.scale_size[1]))
            depth_mask = cv2.resize(depth_mask, (self.scale_size[0], self.scale_size[1]))
        ...
        return {'img_path': img_path, 'image': image, 'depth': depth, 'filename': filename, 'class_id': 0, 'depth_mask':depth_mask}
